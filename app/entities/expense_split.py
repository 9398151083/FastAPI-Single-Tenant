from datetime import datetime
import uuid
import sqlalchemy as sa
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy import ForeignKey
from app.connectors.database_connector import Base


class ExpenseSplit(Base):
    """
    ✅ PERFECT for Splitwise - Who owes who for expenses
    """

    __tablename__ = "expense_splits"

    id: uuid.UUID = sa.Column(UUID(as_uuid=False), primary_key=True, nullable=False)

    expense_id: uuid.UUID = sa.Column(
        UUID(as_uuid=False), ForeignKey("expenses.id"), nullable=False, index=True
    )
    group_id: uuid.UUID = sa.Column(
        UUID(as_uuid=False), nullable=False, index=True
    )  # ✅ ADDED

    # ✅ KEY FIELDS FOR SPLITWISE:
    owes_user_id: uuid.UUID = sa.Column(
        UUID(as_uuid=False), nullable=False, index=True
    )  # Who OWES
    owed_user_id: uuid.UUID = sa.Column(
        UUID(as_uuid=False), nullable=False, index=True
    )  # Who is OWED

    amount = sa.Column(sa.Numeric(10, 2), nullable=False)
    is_settled: bool = sa.Column(sa.Boolean, nullable=False, default=False)

    created_at = sa.Column(sa.DateTime, nullable=False, default=sa.func.now())
